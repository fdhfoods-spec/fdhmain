    ...
    <div className="min-h-screen bg-background text-foreground font-sans pb-20">
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-border pb-4">
          <div>
            <h1 className="text-xl font-black text-foreground">Your Bookmarked Items</h1>
            <p className="text-xs text-muted-foreground">Items saved for fast re-ordering.</p>
          </div>
          <span className="text-xs font-mono font-bold text-foreground/70">{bookmarkedProducts.length} items</span>
        </div>

        {bookmarkedProducts.length === 0 ? (
          <div className="bg-card border border-border rounded-3xl p-12 text-center space-y-3 shadow-xl">
            ...
            <p className="text-xs text-muted-foreground max-w-sm mx-auto">Explore our fresh cuts and tap the heart icon on any item to bookmark it here!</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {bookmarkedProducts.map((product) => (
              <div key={product.id} className="bg-card border border-border rounded-3xl p-4 flex flex-col justify-between shadow-xl group hover:border-secondary transition-all">
                <div>
                  <div className="aspect-square relative rounded-2xl overflow-hidden bg-muted mb-3">
                    <Image src={product.image} alt={product.name} fill className="object-cover group-hover:scale-105 transition-transform" />
                    <button
                      onClick={() => toggleWishlist(product.id)}
                      className="absolute top-3 right-3 p-2 bg-muted/80 backdrop-blur-md rounded-xl text-destructive border border-destructive/20 hover:scale-110 transition-transform"
                    >
                ...
                <div className="border-t border-border pt-3 mt-3 flex items-center justify-between">
                  <div>
                    <span className="text-[10px] text-muted-foreground block font-bold">{product.weight}</span>
                    <span className="text-base font-mono font-black text-foreground">₹{product.price}</span>
                  </div>
